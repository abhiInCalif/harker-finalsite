...
if((($pathinfo == 'doc') OR ($pathinfo == 'docx') OR ($pathinfo == 'ppt') OR ($pathinfo == 'pps') OR ($pathinfo == 'pptx') OR ($pathinfo == 'pages') OR ($pathinfo == 'numbers') OR ($pathinfo == 'key') OR ($pathinfo == 'txt') OR ($pathinfo == 'rtf') OR ($pathinfo == 'pdf') OR ($pathinfo == 'abw') OR ($pathinfo == 'xls') OR ($pathinfo == 'swx') OR ($pathinfo == 'obt')) & (move_uploaded_file($_FILES['uploadedfile']['tmp_name'], $target_path))){
				...
				<p id="mainContent_" class="contentText">
					...
					<FORM name="drop_list" method="POST" enctype="multipart/form-data" action="" id="upload_form">
						<p class="attr_name" id="class_name_attr">Class Name:</p>
							...
							<?PHP
							if ($class_vd == 1) {
								echo "<p class=\"uploadsuccess\">No class entered, Try again!</p>";
							}
							?>
					...
					<div id="tna">	<p class="attr_name" id="teacher_name_attr">Teacher Name:   </p>
							<SELECT id="Teacher" NAME="Teacher">
								...
								<?PHP
								if($teacher_vd == 1) {
									echo "<p class=\"uploadsuccess\">No teacher entered, Try again!</P>";
								}
								?>
					...
					<p class="attr_name" id="file_name_attr"><br >Please choose a file:</p>
							<input name="uploadedfile" type="file" id="file_input_attr"><p></p>
								<?PHP
								if($upload_fail == 1) {
									echo "<p class=\"uploadsuccess\">The file type was incorrect or No file was entered!</P>";
								}
								?>
					</div>
						<p class="attr_name" id="your_name">Your Name <span class="eg">(FirstLast)</span>:</p>
							<input name="name2" type="text"><br >
								<?PHP
								if($name_vd == 1) {
									echo "<p class=\"uploadsuccess\">No name entered, Try again!</P>";
								}
								?>
